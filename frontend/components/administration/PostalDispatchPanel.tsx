...
export function PostalDispatchPanel() {
  ...
  const load = async () => {
    try {
      ...
      setError("Unable to load postal dispatch records.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        setSuccess("Postal dispatch saved successfully.");
      }
      reset();
      await load();
    } catch {
      setError(editingId ? "Unable to update postal dispatch." : "Unable to save postal dispatch.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const remove = async (id: number) => {
    const ok = window.confirm("Are you sure to delete this postal dispatch entry?");
    if (!ok) return;
    try {
      setBusyId(id);
      setError("");
      ...
      setSuccess("Postal dispatch deleted.");
    } catch {
      setError("Unable to delete postal dispatch.");
    } finally {
      setBusyId(null);
    }
  };
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((row) =>
      [row.to_title, row.reference_no, row.address, row.from_title, row.note || ""].join(" ").toLowerCase().includes(q),
    );
  }, [items, search]);
        ...
        <div className="container-fluid p-0">
          <div style={{ display: "grid", gridTemplateColumns: "320px 1fr", gap: 12 }}>
            ...
            <div className="white-box" style={boxStyle()}>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  ...
                  <tbody>
                    {!loading && filtered.length === 0 ? (
                      <tr><td colSpan={7} style={{ padding: 12, color: "var(--text-muted)" }}>No postal dispatch records found.</td></tr>
                    ) : (
                      filtered.map((row) => (
                        <tr key={row.id}>
              ...
              {loading && <p style={{ marginTop: 10, color: "var(--text-muted)" }}>Loading postal dispatch...</p>}
