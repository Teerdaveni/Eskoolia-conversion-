...
export default function ExamPlanAdmitCardPanel() {
  ...
  const loadIndex = async () => {
    const data = await apiGet<{ exams: Exam[]; classes: SchoolClass[]; sections: Section[] }>("/api/v1/exams/exam-plan/admit-card/");
    setExams(data.exams || []);
  ...
  const loadSetting = async () => {
    const data = await apiGet<{ setting: Setting }>("/api/v1/exams/exam-plan/admit-card/setting/");
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        setError(e instanceof Error ? e.message : "Failed to load admit card page.");
      }
    };
    void init();
  }, []);
  ...
  const search = async () => {
    ...
    try {
      setError("");
      setMessage("");
      const data = await apiPost<{ records: StudentRow[]; old_admit_ids: number[] }>("/api/v1/exams/exam-plan/admit-card/search/", {
        exam: Number(examId),
        class: Number(classId),
        section: Number(sectionId),
      });
      setRows(data.records || []);
      setOldIds(data.old_admit_ids || []);
      const next: Record<number, boolean> = {};
      (data.records || []).forEach((r) => {
        next[r.student_record_id] = Boolean((data.old_admit_ids || []).includes(r.student_record_id));
      });
      setSelectedMap(next);
    } catch (e) {
      setRows([]);
  ...
  const generate = async () => {
    try {
      ...
      await apiPost("/api/v1/exams/exam-plan/admit-card/generate/", {
  ...
  return (
    <div className="legacy-panel">
      <section className="sms-breadcrumb mb-20"><div className="container-fluid"><h1 style={{ margin: 0, fontSize: 24 }}>Exam Plan - Admit Card</h1></div></section>
      <section className="admin-visitor-area up_st_admin_visitor"><div className="container-fluid p-0">
        ...
        <div className="white-box" style={{ ...boxStyle(), marginBottom: 12 }}>
          <h3 style={{ marginTop: 0, marginBottom: 12 }}>Admit Card Setting</h3>
          <p style={{ margin: 0, color: "var(--muted)" }}>
            Layout: {setting?.admit_layout || 1}, Student Name: {setting?.student_name ? "Yes" : "No"}, Admission No: {setting?.admission_no ? "Yes" : "No"}
          </p>
        </div>
