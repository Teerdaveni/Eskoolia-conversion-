...
export default function FeesDueReportPage() {
  ...
  const search = async () => {
    ...
    setError("");
    try {
      const data = await apiFetch<{ count: number; results: DueRow[] }>(`/api/v1/reports/fees-due/?${buildParams()}`);
  ...
  const exportCsv = () => {
    const token = getAccessToken();
    window.open(`${API_BASE_URL}/api/v1/reports/fees-due/?${buildParams()}&format=csv&token=${token ?? ""}`, "_blank");
  };
  ...
  return (
    <div>
      <section style={{ background: "#fff", padding: "12px 20px", borderRadius: 8, marginBottom: 20, boxShadow: "0 1px 3px rgba(0,0,0,.08)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <h1 style={{ margin: 0, fontSize: 20, fontWeight: 600 }}>Fees Due Report</h1>
        <div style={{ fontSize: 13, color: "#6b7280", display: "flex", gap: 6 }}>
          ...
          <span>/</span>
          <span style={{ color: "var(--primary, #3b82f6)" }}>Fees Due Report</span>
        </div>
      </section>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 12 }}>
          ...
          <div>
            ...
            <select style={field} value={filters.status} onChange={(e) => setFilters({ ...filters, status: e.target.value })}>
              <option value="">All (Unpaid + Partial)</option>
      ...
      {searched && (
        <section style={{ background: "#fff", borderRadius: 8, padding: 20, boxShadow: "0 1px 3px rgba(0,0,0,.08)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
            <h3 style={{ margin: 0, fontSize: 15, fontWeight: 600 }}>Due Fees</h3>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr style={{ background: "#f9fafb" }}>
                  {["#", "Admission No", "Student Name", "Class", "Section", "Fees Type", "Due Date", "Amount", "Status"].map((h) => (
              ...
              <tbody>
                {rows.length === 0 ? (
                  <tr><td colSpan={9} style={{ padding: 16, color: "#9ca3af", textAlign: "center" }}>No due fees found.</td></tr>
                ) : rows.map((r, i) => {
                  const style = STATUS_STYLE[r.status] ?? { bg: "#f3f4f6", color: "#374151" };
                  return (
                    <tr key={r.assignment_id} style={{ background: i % 2 === 0 ? "#fff" : "#f9fafb" }}>
                      ...
                      <td style={{ padding: "8px 12px", border: "1px solid #e5e7eb" }}>{r.due_date}</td>
                      <td style={{ padding: "8px 12px", border: "1px solid #e5e7eb", fontWeight: 600, color: "#991b1b" }}>{r.amount}</td>
                      <td style={{ padding: "8px 12px", border: "1px solid #e5e7eb" }}>
                        <span style={{ background: style.bg, color: style.color, padding: "2px 8px", borderRadius: 12, fontSize: 12, fontWeight: 600, textTransform: "capitalize" }}>{r.status}</span>
                      </td>
                    </tr>
                  );
                })}
